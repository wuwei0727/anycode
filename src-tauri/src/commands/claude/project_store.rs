...
impl ProjectStore {
    ...
    pub fn list_projects(&self) -> Result<Vec<Project>, String> {
        ...
        let normalize_path = |p: &str| -> String { p.replace('\\', "/").trim_end_matches('/').to_lowercase() };
        ...
        if projects_dir.exists() {
            ...
            for entry in entries {
                ...
                if path.is_dir() {
                    ...
                    // On Windows, sessions can contain WSL-style paths (e.g. /mnt/d/...)
                    // Convert them so existence checks work as users expect.
                    #[cfg(target_os = "windows")]
                    let project_path = {
                        if project_path.starts_with("/mnt/") {
                            crate::commands::wsl_utils::wsl_to_windows_path(&project_path)
                        } else {
                            project_path
                        }
                    };
        ...
        for entry in entries {
            ...
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
                if let Some(session_id) = path.file_stem().and_then(|s| s.to_str()) {
                    ...
                    let first_message = metadata.first_message;
                    let message_timestamp = metadata.message_timestamp;
