...
pub fn build_execution_args(
    ...
    // 🔥 新增：添加禁用的 MCP 服务器参数
    // 注意：--disable-mcp-server 参数需要 Claude CLI 支持
    // 如果 CLI 版本不支持此参数，命令可能会失败并显示 "unknown flag" 错误
    let disabled_servers = crate::commands::mcp::get_disabled_mcp_servers_for_project(project_path);
    if !disabled_servers.is_empty() {
        log::info!("[MCP] Disabling {} MCP servers: {:?}", disabled_servers.len(), disabled_servers);
        log::info!("[MCP] Note: --disable-mcp-server requires Claude CLI support. If you see 'unknown flag' errors, your CLI version may not support this feature.");
        for server_name in disabled_servers {
            args.push("--disable-mcp-server".to_string());
            args.push(server_name);
        }
    } else {
        log::info!("[MCP] No MCP servers to disable for project: {}", project_path);
    }
